...
function Profile() {
  ...
  const orderUrl = `/wp-json/wcm/api/orders`;
  const Userurl = `/wp-json/wp/v2/users/${currentUser.user.id}`;
  useEffect(() => {
    loadProgressBar();
    axios
      .get(orderUrl, {
  ...
  const handleUpdata = (e) => {
    ...
    if (display_name !== "" && user_email !== "") {
      const url = "/wp-json/wp/v2/update-profile";
  ...
  const handlePassword = (e) => {
    ...
    const url = "/wp-json/wp/v2/update-pass";
